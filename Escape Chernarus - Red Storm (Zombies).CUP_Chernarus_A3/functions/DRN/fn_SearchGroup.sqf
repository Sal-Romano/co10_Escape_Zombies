...
while {!_exitScript} do {
    ...
    //diag_log format ["SEARCH GROUP: _group: %1 units _group: %2 isNull _group: %3",_group,units _group, isNull _group];

    _currentPos = position (units _group select 0); // FIXME Error Undefined variable in expression: _currentpos
