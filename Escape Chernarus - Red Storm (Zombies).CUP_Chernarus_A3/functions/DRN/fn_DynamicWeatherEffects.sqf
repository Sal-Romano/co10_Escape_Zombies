...
private ["_minimumFogDensity", "_maximumFogDensity", "_minimumOvercast", "_maximumOvercast", "_minimumRain", "_maximumRain", "_minimumWind", "_maximumWind", "_minRainIntervalTimeMin", "_maxRainIntervalTimeMin", "_forceRainToStopAfterOneRainInterval", "_maxWind"];
...
// Fog intensity never falls below this value. Must be between 0 and 1 and less than or equal to _maximumFogDensity
// (0 = no fog, 1 = pea soup). (Suggested value: 0).
_minimumFogDensity = 0;

// Fog intensity never exceeds this value. Must be between 0 and 1 and greater than or equal to _minimumFogDensity
// (0 = no fog, 1 = pea soup). (Suggested value: 0.8).
_maximumFogDensity = 0.8;

_minimumFogDecay = 0.001;
_maximumFogDecay = 0.05;

_minimumFogBase = 0.0;
_maximumFogBase = 300.0;
...
// special effects for Nam
if (A3E_WorldName == "Cam Lao Nam") then
{
    _minimumFogDensity = 0.03;
    _maximumFogDensity = 0.5;
...
if (isServer) then {
    ...
    if (_initialFog == -1) then {
        _initialFog = (_minimumFogDensity + random (_maximumFogDensity - _minimumFogDensity));
    }
    else {
        if (_initialFog < _minimumFogDensity) then {
            _initialFog = _minimumFogDensity;
        };
        if (_initialFog > _maximumFogDensity) then {
            _initialFog = _maximumFogDensity;
        };
    };
    ...
    [_minWeatherChangeTimeMin, _maxWeatherChangeTimeMin, _minTimeBetweenWeatherChangesMin, _maxTimeBetweenWeatherChangesMin, _minimumFogDensity, _maximumFogDensity, _minimumOvercast, _maximumOvercast, _minimumWind, _maximumWind, _windChangeProbability] spawn {
        private ["_minWeatherChangeTimeMin", "_maxWeatherChangeTimeMin", "_minTimeBetweenWeatherChangesMin", "_maxTimeBetweenWeatherChangesMin", "_minimumFogDensity", "_maximumFogDensity", "_minimumOvercast", "_maximumOvercast", "_minimumWind", "_maximumWind", "_windChangeProbability"];
        ...
        _minimumFogDensity = _this select 4;
        _maximumFogDensity = _this select 5;
        ...
        while {true} do {
            ...
            if (_minimumFogDensity == _maximumFogDensity && _minimumOvercast != _maximumOvercast) then {
            ...
            if (_weatherType == "FOG") then {
                ...
                if (_fogLevel == 0) then {
                    drn_DynamicWeather_WeatherTargetValue = _minimumFogDensity + (_maximumFogDensity - _minimumFogDensity) * random 0.05;
                };
                if (_fogLevel == 1) then {
                    drn_DynamicWeather_WeatherTargetValue = _minimumFogDensity + (_maximumFogDensity - _minimumFogDensity) * (0.05 + random 0.2);
                };
                if (_fogLevel == 2) then {
                    drn_DynamicWeather_WeatherTargetValue = _minimumFogDensity + (_maximumFogDensity - _minimumFogDensity) * (0.25 + random 0.3);
                };
                if (_fogLevel == 3) then {
                    drn_DynamicWeather_WeatherTargetValue = _minimumFogDensity + (_maximumFogDensity - _minimumFogDensity) * (0.55 + random 0.45);
                };
