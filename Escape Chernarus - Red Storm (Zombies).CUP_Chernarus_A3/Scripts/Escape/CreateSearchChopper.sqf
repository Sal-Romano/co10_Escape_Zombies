...
if (count _this > 5) then {_minSkill = _this select 5;} else {_minSkill = 0.05;};
if (count _this > 6) then {_maxSkill = _this select 6;} else {_maxSkill = 0.1;};
...
//private _classList = [ a3e_arr_Escape_InfantryTypes] select (side _group == A3E_VAR_Side_Opfor);
{
	if !(_chopper lockedTurret _x) then {
		private _soldier = _group createUnit [selectRandom a3e_arr_Escape_InfantryTypes, [0,0,0], [], 0, "NONE"];
