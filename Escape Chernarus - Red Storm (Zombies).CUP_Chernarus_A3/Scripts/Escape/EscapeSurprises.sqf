...
// Trackers from SOG
if ((A3E_Param_SearchChopper == 3 && A3E_Param_UseDLCSOGPF == 1) || A3E_Debug) then
{
    _surpriseArgs = [_minEnemySkill, _maxEnemySkill];
    _timeInSek = 20 * 60 + random (20 * 60);
    _timeInSek = time + (_timeInSek * (0.5 + (4 - _enemyFrequency) / 4));
    _surprise = ["TRACKERS", _timeInSek, {[drn_searchAreaMarkerName] call drn_fnc_CL_MarkerExists}, false, _surpriseArgs];
    _surprises pushBack _surprise;
    diag_log ("ESCAPE SURPRISE: " + str _surprise);  
};
        ...
        if (!_isExecuted && time > _surpriseTimeSec) then {
            ...
            if (call _condition) then {
                ...
                if (_surpriseID == "RUSSIANSEARCHCHOPPER") then {
                    ...
                    //"O_Pilot_F" createUnit [[0, 0, 30], _group, "", (_minEnemySkill + random (_maxEnemySkill - _minEnemySkill)), "LIEUTNANT"];
                    //_init = [_crewtype,missionNameSpace getVariable ["ForceFlashlights", false]] call ToothFunctions_fnc_getunitloadout;
                    _soldier = _group createUnit [_crewtype, [0, 0, 30], [], 0, "FORM"];
					_soldier setVariable ["UseGrid", True]; //SALSET
                    //[_soldier, (compileFinal _init)] spawn BIS_fnc_spawn;
                    _soldier = _group createUnit [_crewtype, [0, 0, 30], [], 0, "FORM"];
					_soldier setVariable ["UseGrid", True]; //SALSET
                ...
                if (_surpriseID == "TRACKERS") then
                {
                    [drn_searchAreaMarkerName, 1200, A3E_Debug] spawn DRN_fnc_NVATrackers;
                  
                    // Create new tracker surprise
                    _surpriseArgs = [_minEnemySkill, _maxEnemySkill];
                    _timeInSek = 30 * 60 + random (45 * 60);
                    _timeInSek = time + (_timeInSek * (0.5 + (4 - _enemyFrequency) / 4));
                    _surprise = ["TRACKERS", _timeInSek, {[drn_searchAreaMarkerName] call drn_fnc_CL_MarkerExists}, false, _surpriseArgs];
				...
				if (_surpriseID == "LEAFLETDRONE") then {
					...
					_arr = [(getMarkerPos "drn_russianSearchChopperStartPosMarker"), 0, selectRandom a3e_arr_searchChopperEasy, A3E_VAR_Side_Ind] call bis_fnc_spawnvehicle;
