...
drn_fnc_Escape_PopulateVehicle = {
    ...
    while {_continue && (_soldierCount <= _maxSoldiersCount)} do {
        ...
        //_init = [_unitType,missionNameSpace getVariable ["ForceFlashlights", false]] call ToothFunctions_fnc_getunitloadout;
        _insurgentSoldier = _group createUnit [_unitType, [0,0,0], [], 0, "FORM"];
        //[_insurgentSoldier, (compileFinal _init)] spawn BIS_fnc_spawn;
        _insurgentSoldier spawn A3E_fnc_onEnemySoldierSpawn;
        
		_insurgentSoldier setVariable ["UseGrid", True]; //SALSET
