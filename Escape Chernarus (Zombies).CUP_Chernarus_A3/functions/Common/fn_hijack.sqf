...
{
	params ["_target"];
	if (Tooth_radioCall != "") then
	{
		_trigger = createTrigger ["EmptyDetector", ((getposASL _target) vectorAdd [0,0,0]), true];
		_trigger setTriggerArea [25, 25, 0, false];
		_trigger setTriggerActivation ["NONE", "PRESENT", false];
		_trigger setSoundEffect ["$NONE$", "", "", Tooth_radioCall];
	};
};
...
if (_count > 0 && _isUnconscious) exitWith {
    cutText ["Radio call aborted!", "Plain", 1];
