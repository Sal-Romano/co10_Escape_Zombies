...
private["_obj","_pos"];
params ["_center","_rotation","_backpack","_light"];

//Stuff that needs to be global
if(isserver) then {
	[_center,10] call a3e_fnc_cleanupTerrain;
	...
	_pos = [_center,_center vectorAdd [random  0.2, random 0.2,0],_rotation] call A3E_fnc_rotatePosition;
	_backpack setdir ((getdir _backpack) + _rotation);
	_backpack setPosATL _pos;
	diag_log format ["A3E_PrisonBackpack moved to %1 ", getPos _backpack];
...
_obj = Tooth_enemyFlagPoleMilitia createVehicleLocal _pos;
