...
drn_fnc_CL_RunGarbageCollector = {
    private ["_referenceGroup", "_cleanUpDistance", "_cleanUpTime", "_debug"];
    private ["_emptyGroups", "_groupsToDelete", "_unitsToDelete"];
    
    _referenceGroup = _this select 0;
    if (count _this > 1) then {_cleanUpDistance = _this select 1;} else {_cleanUpDistance = 500};
    if (count _this > 2) then {_cleanUpTime = _this select 2;} else {_cleanUpTime = (5*60)};
    ...
    while {true} do {
        ...
        {
            if (count units _x <= 0) then {
                _groupsToDelete pushBackUnique _x;
            };
        ...
        {
            _unit = _x;
            _index = a3e_var_CL_GarbageCollectorUnits find _unit;
            a3e_var_CL_GarbageCollectorUnits deleteAt _index;
            ["Unit '" + str _unit + "' garbage collected."] call drn_fnc_CL_ShowDebugTextAllClients;
            deleteVehicle _unit;
