        ...
        if (alive _reportingUnit) then {
            if (diag_tickTime > _reportingStartTime + _timeUntilReportToHQSec) then {
				...
				if(count(_detectedUnitsPosition nearObjects [_knownPositionHelperObject, _knownPositionMinDistance])==0) then {
					...
					if((diag_tickTime-_firstsight)>=_artilleryTimeThreshold && (diag_tickTime > (_artilleryCooldown+_lastArtilleryStrike))) then {
						if(random 100 < a3e_var_artillery_chance) then {
							...
							if(random 100 < 80 || count a3e_arr_CASplane == 0) then {
								...
								{
									_errorRadius = 150.0;
									_targetPos = getPos p1;
									_position = [(_targetPos select 0) - _errorRadius + (2 * random _errorRadius), (_targetPos select 1) - _errorRadius + (2 * random _errorRadius), 0];
									_cas = _group createUnit [a3e_CASModule,getpos (_list select 0) , [], 0, ""]; 
                                    ...
                                    _cas setVariable ["BIS_fnc_initModules_disableAutoActivation",false,true];
									_strikesuccess = true;
								};
				...
				if (!_searchAreaMarkerCreated) then {
					...
					//_marker setMarkerTextLocal "SEARCH AREA";
					//_marker setMarkerTextLocal "SEARCH AREA";
