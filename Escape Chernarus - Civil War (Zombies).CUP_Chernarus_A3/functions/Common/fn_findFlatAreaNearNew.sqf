...
// for a flat area within a grid of +/- _maxDist. The "radius"
...
//
//   _maxDist         (default is 1000):  the radius of the search area
...
params [["_initialPos", [0,0,0]],
        ["_flat_area_radius", 4.0],
        ["_minDist", 0.0],
        ["_maxDist", 300],
        ["_gradient", 0.1],
        ["_objectDistance", 5.0],
...
while {_keepSearching} do { // NEW
    ...
    // NEW
    _final_pos = [_initialPos, _minDist, _maxDist, _objectDistance, 0, _gradient, 0, [], [0,0,0]] call BIS_fnc_findSafePos;
    ...
    if (_ii > _max_num_search_areas) then {
        ...
        //diag_log format ["fn_findFlatAreaNearNew _max_num_search_areas_excceded!!!!!!!!!: %1", _ii];
    };
...
diag_log format ["fn_findFlatAreaNearNew _retval: %1 _initialPos: %2", _retval, _initialPos];
_retval;