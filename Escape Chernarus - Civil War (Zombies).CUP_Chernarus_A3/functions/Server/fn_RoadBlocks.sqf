...
/*
	...
	if (count a3e_arr_Escape_RoadBlock_StaticTypes_Low > 0) then
    {
	
        private _gun = selectRandom a3e_arr_Escape_RoadBlock_StaticTypes_Low;
       // private _staticLow = [_gun, (_pos vectoradd [10,0,0]), _dir, _centerPos, _rotateDir] call _fnc_CreateObject;
    	private _staticLow = createVehicle [_gun, (_pos vectoradd [10,0,0]), [], 0, "NONE"];
    	[_staticLow] call a3e_fnc_onVehicleSpawn;
    	_vehicles pushBack _staticLow;
    	_units pushback ([_staticLow,_side] call A3E_fnc_AddStaticGunner);
    };

    if (count a3e_arr_Escape_RoadBlock_StaticTypes_Low > 0) then
    {
    
        private _gun = selectRandom a3e_arr_Escape_RoadBlock_StaticTypes_High;
       // private _staticHigh = [_gun, (_pos vectoradd [-10,0,0]), _dir, _centerPos, _rotateDir] call _fnc_CreateObject;
        private _staticHigh = createVehicle [_gun, (_pos vectoradd [-10,0,0]), [], 0, "NONE"];
        [_staticHigh] call a3e_fnc_onVehicleSpawn;
        _vehicles pushBack _staticHigh;
        _units pushback ([_staticHigh,_side] call A3E_fnc_AddStaticGunner);
    };
*/

    _units = ([_pos, _dir + _angle, a3e_arr_Escape_RoadBlock_StaticTypes_Low, a3e_arr_Escape_RoadBlock_StaticTypes_High, _possibleVehicles, _side] call selectRandom [A3E_fnc_ToothRoadblock1]); //, A3E_fnc_ToothRoadblock2, A3E_fnc_ToothRoadblock3, A3E_fnc_ToothRoadblock4]);
	...
	_group setVariable ["UseGrid", True]; //SALSET
    _group createUnit [selectRandom _guardTypes, _pos, [], 0, "FORM"];
	_group setVariable ["UseGrid", True]; //SALSET
    _group createUnit [selectRandom _guardTypes, _pos, [], 0, "FORM"];
	_group setVariable ["UseGrid", True]; //SALSET
    ...
    (_units select 0) append (units _group);
    ...
    //[_units,_vehicles];
    diag_log format ["RoadBlocks: _units: '%1'", _units];
    _units;
};
