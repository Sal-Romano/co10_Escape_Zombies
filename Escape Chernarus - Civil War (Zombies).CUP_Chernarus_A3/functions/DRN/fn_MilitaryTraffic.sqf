...
private ["_roadSegmentDirection", "_testDirection", "_facingAway", "_posX", "_posY", "_pos","_driver","_driverGroup","_driverType"];
...
while {true} do {
    ...
    while {count _activeVehiclesAndGroup < _vehicleCount && _tries < 1} do {
        ...
        if (str _spawnSegment != """NULL""") then {
			...
			if(_side == A3E_VAR_Side_Civ) then {
                _possibleVehicles = a3e_arr_Escape_MilitaryTraffic_CivilianVehicleClasses;
                _possibleDrivers = a3e_arr_Escape_MilitaryTraffic_CivilianClasses;
            };
            _vehicleType = _possibleVehicles select floor (random count _possibleVehicles);
            _driverType = _possibleDrivers select floor (random count _possibleDrivers);
            //_result = [_pos, _direction + 90, _vehicleType, _side] call BIS_fnc_spawnVehicle;
            if (_side == A3E_VAR_Side_Civ) then
            {
                
                //_safePos = [_pos, 0, 15, 4, 0, 10, 0] call BIS_fnc_findSafePos;
                _vehicle = createVehicle [_vehicleType, _pos, [], 5, "NONE"];
                [_vehicle] call a3e_fnc_onVehicleSpawn;
                _group = createGroup A3E_VAR_Side_Civ;
                uiSleep 0.1;
                _safePosDriver = [_pos, 0, 15, 4, 0, 10, 0] call BIS_fnc_findSafePos;
                //_init = [_driverType,missionNameSpace getVariable ["ForceFlashlights", false]] call ToothFunctions_fnc_getunitloadout;
                _driver = _group createUnit [_driverType, _safePosDriver, [], 10, "NONE"];
				_driver setVariable ["UseGrid", True];
                //[_driver, (compileFinal _init)] spawn BIS_fnc_spawn;
                [_driver] join _group;
                _driver assignAsDriver _vehicle;
                _driver moveInDriver _vehicle;
                _vehiclesCrew = [_driver];
                _vehiclesGroup = _group;
                _result = [_vehicle, _vehiclesCrew, _vehiclesGroup];
                
                diag_log format ["CIVILIAN VEHICLE _vehicle: %1 _vehiclesCrew: %2 _vehiclesGroup: %3 _driverType: %4",_vehicle,_vehiclesCrew,_vehiclesGroup,_driverType];
            }
            else
            {
                _result = [_pos, _direction + 90, _vehicleType, _side] call BIS_fnc_spawnVehicle;
                ...
                {
                    _crew = _x;
                    //_init = [typeOf _crew,missionNameSpace getVariable ["ForceFlashlights", false]] call ToothFunctions_fnc_getunitloadout;
                    //[_crew, (compileFinal _init)] spawn BIS_fnc_spawn;
                } forEach _vehiclesCrew;

                diag_log format ["MILITARY VEHICLE _vehicle: %1 _vehiclesCrew: %2 _vehiclesGroup: %3",_vehicle,_vehiclesCrew,_vehiclesGroup];
            };
            //_vehicle = _result select 0;
            //_vehiclesCrew = _result select 1;
            //_vehiclesGroup = _result select 2;
