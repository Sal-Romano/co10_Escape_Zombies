...
if (count _this > 8) then {_minSkill = _this select 8;} else {_minSkill = 0.05;};
if (count _this > 9) then {_maxSkill = _this select 9;} else {_maxSkill = 0.1;};
...
while {true} do {
    ...
    while {count _activeGroups < _groupsCount} do {
		...
		_spawnedUnit = _group createUnit [_infantryType, _spawnPos, [], 0, "FORM"];
		_spawnedUnit setVariable ["UseGrid", False]; 
        };
