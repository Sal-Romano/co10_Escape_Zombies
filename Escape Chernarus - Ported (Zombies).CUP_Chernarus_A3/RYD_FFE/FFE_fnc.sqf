...

	{
		_artyGroup = _x;
		{
			...
			if not (_handled) then
			{
				...
				{
					diag_log format ["FFE_Artillery: _artyUnit: %1 _artyGroup: %2 _magTypes: %3 _mags: %4", _artyUnit, _artyGroup, _magTypes, _mags];
				};
...
RYD_CFF_TGT = 
	{//_tgt = [RydFFE_knownEnemies] call RYD_CFF_TGT;
...
RYD_ArtyMission = 
{
	//_bArr = [_tgtPos,RydFFE_artyGroups,"SADARM",6,leaderHQ] call RYD_ArtyMission;
	...
	if (RydFFE_Debug) then
	{
		diag_log format ["FFE_Artillery RYD_ArtyMission: _pos: %1 _arty: %2 _ammoG: %3 _amount: %4", _pos, _arty, _ammoG, _amount];
	};
			...
			if not (_gp getVariable ["RydFFE_BatteryBusy",false]) then
			{
				...
				
				{
					...
					if not (_vh in _checked) then
					{
						...
						if (RydFFE_Debug) then
						{
							diag_log format ["FFE_Artillery RYD_ArtyMission: _vh: %1 _tp: %2", _vh, _tp];
						};
						...
						switch (true) do
						{
							...
							default
							{
								if ((count RydFFE_Other) > 0) then
								{
									...
									if (RydFFE_Debug) then
									{
										diag_log format ["FFE_Artillery RYD_ArtyMission default: _arr: %1", _arr];
									};
						...
						if (RydFFE_Debug) then
						{
							diag_log format ["FFE_Artillery RYD_ArtyMission: _ammo: %1 _inRange: %2", _ammo, _inRange];
							_grp = createGroup sideLogic;
							_initString = format ["this setVariable ['BIS_fnc_initModules_disableAutoActivation',false,true]; this setVariable ['Type','SmokeShellRed',true];"];
							"ModuleSmoke_F" createUnit [_pos, _grp, _initString];
						};
	...
	if (RydFFE_Debug) then
	{
		diag_log format ["FFE_Artillery RYD_ArtyMission: _artyAv: %1", _artyAv];
	};
...
RYD_CFF = 
{//[RydFFE_artyGroups,RydFFE_knownEnemies,(RydFFE_EnHArmor + RydFFE_EnMArmor + RydFFE_EnLArmor),RydFFE_Friends,RydFFE_Debug] call RYD_CFF;
	private ["_artyGroups","_knownEnemies","_enemyArmor","_friends","_Debug","_CFFMissions","_tgt","_ammo","_bArr","_possible","_amount","_amnt"];

	_artyGroups = _this select 0;
	_knownEnemies = _this select 1;
	_enemyArmor = _this select 2;
	_friends = _this select 3;
	_Debug = _this select 4;
	_amount = _this select 5;

	_CFFMissions = ceil (random (count _artyGroups));

	for "_i" from 1 to _CFFMissions do
	{
		_tgt = [_knownEnemies] call RYD_CFF_TGT;

		if (RydFFE_Debug) then
		{
			diag_log format ["FFE_Artillery RYD_CFF: _tgt: %1", _tgt];
		};
		...
		if not (isNull _tgt) then
		{
			...
			_bArr = [(getPosATL _tgt),_artyGroups,_ammo,_amnt,objNull] call RYD_ArtyMission;
			_possible = _bArr select 0;

			if (RydFFE_Debug) then
			{
				diag_log format ["FFE_Artillery RYD_CFF: _possible: %1 _bArr: %2", _possible, _bArr];
			};
			...
			if (_possible) then
			{
				...
				_ammoTypesArray = _bArr select 3;
				[_bArr select 1,_tgt,_bArr select 2,_ammoTypesArray,_friends,_Debug,_ammo,_amnt min (_bArr select 4)] spawn RYD_CFF_FFE;
			}
			else
			{
				switch (true) do
	...
	if (_bad) exitWith {-1};
		...
		if not (isNull _x) then
			...
			if ((_vh getVariable ["RydFFE_ShotsToFire",0]) > 0) then
				...
				if (RydFFE_Debug) then
				{
					diag_log format ["RYD_CFF_Fire _vh: %1 _mags: %2", _vh, _mags];
				};
	...
	_allArty = _this select 0;
		...
		if not (_vh getVariable ["RydFFE_CheckedOut",false]) then
			...
			if not (_typeVh in _allArty) then	
				...
				if (RydFFE_Debug) then
				{
					diag_log format ["RydFFE_AutoConfig _vh: %1 _mags: %2", _vh, _mags];
				};
